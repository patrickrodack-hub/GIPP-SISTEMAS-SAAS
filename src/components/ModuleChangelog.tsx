...
const ModuleChangelog = () => (
    <div className="glass-modern p-10 rounded-[2.5rem] animate-entrance max-w-3xl mx-auto">
        ...
        <div className="space-y-8">
            ...
            {/* NOVO BLOCO ADICIONADO PARA VERSÃO 13.0.0 */}
            <div className="relative pl-8 border-l-2 border-indigo-600 animate-entrance"> 
                ...
                <h3 className="font-extrabold text-lg text-indigo-950 font-[Outfit]">v13.0.0 - Auditoria SaaS de Hardware e Aparelhos Conectados em Tempo Real, Telemetria Autêntica de Rede, E-books Teológicos Dogmáticos e Consolidação Ministerial</h3>
                <p className="text-xs text-indigo-600 font-black uppercase mb-3 tracking-wider">Outubro 2026 (Atual)</p>
                <ul className="list-disc pl-4 space-y-2 text-slate-600 text-sm">
                    <li><strong className="text-slate-700 font-sans font-extrabold">Auditoria de Aparelhos SaaS 100% Real (Zero Simulações):</strong> Eliminação total de dados de teste e simulações. Implementação de camada de telemetria baseada em persistência Firestore (<code className="text-indigo-700 font-bold bg-indigo-50 px-1 py-0.5 rounded text-xs">dispositivos_conectados</code>) com sincronização em tempo real (<code className="text-indigo-700 font-bold bg-indigo-50 px-1 py-0.5 rounded text-xs">onSnapshot</code>), heartbeat persistente a cada 60 segundos, e detecção matemática de hardware (Windows, macOS, Linux, Android, iOS, ChromeOS).</li>
                    <li><strong className="text-slate-700 font-sans font-extrabold">Resolução de IP Público & Geolocalização Física:</strong> Criação de endpoint backend nativo <code className="text-indigo-700 font-bold bg-indigo-50 px-1 py-0.5 rounded text-xs">/api/client-info</code> para identificação precisa do endereço IP público do cliente, provedor de internet e localização geográfica verídica, eliminando mascaramento ou faixas fictícias.</li>
                    <li><strong className="text-slate-700 font-sans font-extrabold">Identificação Automática de Sessão do Operador:</strong> Reconhecimento do dispositivo local ativo com badge animado de pulso (<span className="text-emerald-700 font-black bg-emerald-50 px-1.5 py-0.5 rounded text-xs">Este Aparelho • Online Agora</span>), fornecendo clareza imediata sobre quais terminais pertencem ao próprio operador ou a outros usuários da congregação.</li>
                    <li><strong className="text-slate-700 font-sans font-extrabold">Auditoria Multicanal de Notificações Criptografadas:</strong> Painel de verificação de canais de transmissão push ativos: Web Push Nativo W3C (VAPID / Service Worker), Google Cloud Messaging / FCM e sessões autenticadas do Portal do Membro via PWA, com teste de transmissão e revogação remota de credenciais.</li>
                    <li><strong className="text-slate-700 font-sans font-extrabold">Pareamento Rápido via PWA e QR Code:</strong> Modal responsivo para pareamento e conexão imediata de novos smartphones e tablets através de escaneamento de QR Code ou envio de link seguro com suporte total a Progressive Web App (PWA).</li>
                    <li><strong className="text-slate-700 font-sans font-extrabold">Universidade Teológica, E-books & Apostilas Dogmáticas (CGADB / CPAD):</strong> Integração estrutural com os 24 capítulos da Declaração de Fé das Assembleias de Deus, geração de apostilas com fundamentação bíblica, histórica e aplicação prática, e-books interativos para níveis Básico, Médio e Avançado, quiz avaliativo e certificação.</li>
                    <li><strong className="text-slate-700 font-sans font-extrabold">Consolidação do Ministério de Louvor & Repertório Musical:</strong> Transposição tonal dinâmica (+/- semitons), capotraste inteligente, auto-scroll contínuo com ajuste de velocidade, metrônomo Web Audio API e modo palco escuro (Dark Stage Mode).</li>
                    <li><strong className="text-slate-700 font-sans font-extrabold">Atualização Geral de Pacotes, Planos & Permissões:</strong> Homologação do Módulo de Auditoria de Aparelhos SaaS, E-books de Teologia, Loja Virtual e Louvor em todos os pacotes SaaS (Básico, Standard e Avançado), atualização da matriz de permissões de operadores e elevação global do ecossistema para GIPP v13.0.0 Ultimate Platinum v18.</li>
                </ul>
            </div>

            {/* BLOCO DA VERSÃO 12.0.0 */}
            <div className="relative pl-8 border-l-2 border-slate-300 opacity-90"> 
                ...
                <h3 className="font-extrabold text-lg text-slate-800 font-[Outfit]">v12.0.0 - Ministério de Louvor, Repertório Musical, Cifras Dinâmicas, Capotraste Inteligente, Auto-Scroll e Leitor de Culto ao Vivo</h3>
                <p className="text-xs text-slate-500 font-black uppercase mb-3 tracking-wider">Setembro 2026 (Anterior)</p>
                <ul className="list-disc pl-4 space-y-2 text-slate-600 text-sm">
                    <li><strong className="text-slate-700 font-sans font-extrabold">Auto-Scroll Contínuo com Controle de Velocidade em Tempo Real:</strong> Implementação de rolagem automática fluida na folha de cifra com seletor de velocidades calibradas (0.5x, 1.0x, 1.5x, 2.0x e 3.0x), acionamento rápido por atalho de teclado (Barra de Espaço) e parada inteligente no final da música.</li>
                    <li><strong className="text-slate-700 font-sans font-extrabold">Realce da Linha Ativa & Cursor Vertical de Foco:</strong> Sincronização em tempo real da linha que está sendo executada pelo músico (posicionada a 30% do topo da janela) com barra vertical âmbar de foco e realce de linha, além de navegação instantânea para qualquer linha por toque/clique ou botões no dock.</li>
                    <li><strong className="text-slate-700 font-sans font-extrabold">Dock Flutuante de Controle (`CifraScrollDock`):</strong> Barra de controle flutuante e ergonômica acessível tanto no modo janela quanto em tela cheia com status de reprodução, seletor de velocidade, contador de linha ativa e controles de navegação.</li>
                    <li><strong className="text-slate-700 font-sans font-extrabold">Transposição Harmônica Inteligente & Dissonâncias:</strong> Motor harmônico com transposição instantânea por semitons (+/-), recalculando automaticamente sustenidos, bemóis, graus de acordes e baixos invertidos (ex.: C/E, G/B, F#/A#).</li>
                    <li><strong className="text-slate-700 font-sans font-extrabold">Capotraste com Cálculo de Dedilhado/Formas:</strong> Seletor de afinação de 1ª a 7ª casa com cálculo automático de transposição inversa para violão/guitarra, exibindo as digitações reais para os músicos tocarem com facilidade.</li>
                    <li><strong className="text-slate-700 font-sans font-extrabold">Leitor de Culto ao Vivo (`WorshipLiveReader`) & Modo Palco:</strong> Interface escura de alto contraste (Stage Mode) e opção em 2 colunas para tablets e palcos escuros, permitindo transição contínua entre todas as músicas da setlist do culto com atalhos de setas (esquerda/direita).</li>
                    <li><strong className="text-slate-700 font-sans font-extrabold">Metrônomo Sonoro e Visual Integrado:</strong> Gerador de áudio via Web Audio API com primeiro tempo acentuado, seletor de compassos (2/4, 3/4, 4/4, 6/8), tap tempo e pulso visual síncrono.</li>
                    <li><strong className="text-slate-700 font-sans font-extrabold">Atualização Geral de Pacotes, Planos & Permissões:</strong> Homologação do Módulo de Repertório e Louvor nos planos SaaS (Standard e Avançado), atualização da matriz de permissões de operadores e elevação global do ecossistema para GIPP v12.0.0 Ultimate Platinum v17.</li>
                </ul>
            </div>

            {/* BLOCO DA VERSÃO 11.0.0 */}
            <div className="relative pl-8 border-l-2 border-slate-300 opacity-90"> 
                 <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-slate-400"></div>
                <h3 className="font-extrabold text-lg text-slate-800 font-[Outfit]">v11.0.0 - Loja Virtual Eclesiástica, Emissão de Documentos Fiscais / DAV, Pedidos de Compra com QR Code & Janelas Interativas</h3>
                <p className="text-xs text-slate-500 font-black uppercase mb-3 tracking-wider">Setembro 2026 (Anterior)</p>
                <ul className="list-disc pl-4 space-y-2 text-slate-600 text-sm">
            ...
            <div className="relative pl-8 border-l-2 border-indigo-500 opacity-90"> 
                ...
                <ul className="list-disc pl-4 space-y-2 text-slate-600 text-sm">
                    ...
                    <li><strong className="text-slate-700 font-sans font-extrabold">GIPP Planilhas:</strong> Novo editor de planilhas de alta performance no módulo GIPP Escritório, compatível com as fórmulas e experiência do Google Sheets e Microsoft Excel. Suporta múltiplas abas, barra de fórmulas e salvamento no formato .gplan.</li>
