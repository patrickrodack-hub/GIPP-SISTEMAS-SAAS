...
const newModule = `    {
        ...
        title: 'Declaração de Fé da CPAD',
        ...
        quiz: [
            {
                question: 'Segundo a Declaração de Fé da CPAD, como é identificado o Batismo no Espírito Santo?',
                options: [
...
if (insertIndex > -1) {
    const output = data.slice(0, insertIndex) + (data.charAt(insertIndex - 1) === ',' ? '' : ',') + '\n' + newModule + '\n' + data.slice(insertIndex);
